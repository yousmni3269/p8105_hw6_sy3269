...
baby-specific factors and maternal factors that are explicitly related
to weight. The predictors are as follows: \* bhead: baby’s head
circumference at birth (centimeters) \* blength: baby’s length at birth
(centimeteres) \* gaweeks: gestational age in weeks \* delwt: mother’s
weight at delivery (pounds) \* mheight: mother’s height (inches) \*
ppbmi: mother’s pre-pregnancy BMI \* ppwt: mother’s pre-pregnancy weight
(pounds) \* wtgain: mother’s weight gain during pregnancy (pounds)
...
almost linear relationship. Hence, alias function was used to check the
multilinearity and it was found that `delwt`, `ppwt` and `wtgain` are
correlated.

To improve this model, `delwt`, `ppwt` will be removed and only `wtgain`
will used as a predictor.
...
![](p8105_hw6_sy3269_files/figure-gfm/unnamed-chunk-4-1.png)<!-- -->![](p8105_hw6_sy3269_files/figure-gfm/unnamed-chunk-4-2.png)<!-- -->![](p8105_hw6_sy3269_files/figure-gfm/unnamed-chunk-4-3.png)<!-- -->![](p8105_hw6_sy3269_files/figure-gfm/unnamed-chunk-4-4.png)<!-- -->

Adjusting the predictors did not improve the adjusted R-squared value
but successfully removed the alias. VIF was also checked and with no
outstanding number, there seems to be no multicolinearity issue.
However, residual vs leverage plot is worth noting as it indicates an
influential point on the right upper corner.

To further improve the model, cross validation is performed. The linear
model above is compared with a smooth model.
...
cv_test = 
  cv_df |> 
  mutate(
    linear_mod = map(train, \(df) lm(bwt ~ bhead + blength + gaweeks + mheight + 
                                       ppbmi + wtgain, data = df)), 
    smooth_mod = map(train, \(df) gam(bwt ~ s(bhead) + s(blength) + s(gaweeks) + 
                                        s(mheight) + s(ppbmi) + s(wtgain), data = df))
  ) |>
  mutate(
...
According to the violin plot, smooth model seems to a better option than
the linear one. I will finalize my model as below.

``` r
bwt_model_final = gam(bwt ~ s(bhead) + s(blength) + s(gaweeks) + 
                        s(mheight) + s(ppbmi) + s(wtgain), data = birthweight_df)
...
```
    ...
    ## 
    ## Formula:
    ## bwt ~ s(bhead) + s(blength) + s(gaweeks) + s(mheight) + s(ppbmi) + 
    ##     s(wtgain)
    ## 
    ## Parametric coefficients:
    ##             Estimate Std. Error t value Pr(>|t|)    
    ## (Intercept) 3114.404      4.129   754.3   <2e-16 ***
    ## ---
    ## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
    ## 
    ## Approximate significance of smooth terms:
    ##              edf Ref.df       F p-value    
    ## s(bhead)   6.550  7.470 187.490 < 2e-16 ***
    ## s(blength) 6.956  7.915 201.146 < 2e-16 ***
    ## s(gaweeks) 4.455  5.485  20.029 < 2e-16 ***
    ## s(mheight) 1.000  1.000  60.965 < 2e-16 ***
    ## s(ppbmi)   3.325  4.228   6.296 3.8e-05 ***
    ## s(wtgain)  3.483  4.428  19.753 < 2e-16 ***
    ## ---
    ## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
    ## 
    ## R-sq.(adj) =  0.718   Deviance explained =   72%
    ## GCV =  74473  Scale est. = 74014     n = 4342

Then, `add_predictions` and `add_residuals` are used to find the
residuals and predictions of the finalized model. These values are
plotted against each other.

``` r
birthweight_df |>
  modelr::add_predictions(bwt_model_final) |>
  modelr::add_residuals(bwt_model_final) |>
...
validation. The two other models are as follows: \* One using length at
birth and gestational age as predictors (main effects only) \* One using
...
cv_res_df = 
  cv_df |>
  mutate(
    bwt_my_model = map(train, \(df) gam(bwt ~ s(bhead) + s(blength) + s(gaweeks) + 
                                          s(mheight) + s(ppbmi) + s(wtgain), data = df)),
